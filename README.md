...
## ※ 2번 같은 경우에는 git bash로 git cli환경에서 명령어로 처리해야 한다.

```bash
# 로컬 레포지토리에서
git init  -> 소스트리에서 생성이랑 같은 느낌 

git remote add origin <원격지 주소>  -> 소스트리에서 원격 누르는 것과 유사

git pull origin main 

git add -A  -> 소스트리에서 커밋파트에서 파일 위로 올리는 것
